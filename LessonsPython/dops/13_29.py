...
def create_cars(prices, types, count_cars):
    for _ in range(count_cars):
        ...
        current_type = random.randint(PASSENGER_CAR, TRUCK_CAR)
...
print(f"avg price passenger cars = {get_avg_price_passenger_cars(prices, types, count_cars):.2f}")